...
async function seedAdminSession(context) {
  ...
  await context.addInitScript(([sessionToken, sessionUser]) => {
    try {
      ...
      console.warn('[ADMIN POPUP TEST] Failed to seed session:', error);
    }
  }, [token, user]);
}

async function openAdminOrdersPopup(page, context) {
  const launcherId = await page.evaluate(() => {
    const button = document.createElement('button');
    const id = `admin-orders-popup-launcher-${Date.now()}`;
    button.id = id;
    button.type = 'button';
    button.style.position = 'absolute';
    button.style.left = '-9999px';
    button.textContent = 'Open Admin Orders Popup';
    button.addEventListener('click', () => {
      window.open('/admin/orders.html?popup=1', 'botzzz-admin-orders', 'width=1280,height=820,resizable=yes,scrollbars=yes');
    });
...
async function run() {
  ...
  try {
    ...
    console.log('[TEST] Starting admin orders popup verification...');
    await page.goto(new URL('/admin/orders.html', BASE_URL).toString(), { waitUntil: 'domcontentloaded' });

    const popup = await openAdminOrdersPopup(page, context);
    const popupUrl = popup.url();

    if (!popupUrl.includes('/admin/orders.html') || !popupUrl.includes('popup=1')) {
      throw new Error('Admin orders popup URL missing popup parameter.');
    }

    const popupMode = await popup.evaluate(() => document.body.classList.contains('popup-mode'));
    if (!popupMode) {
      throw new Error('Admin orders popup did not enable popup-mode.');
    }
    ...
    console.log('[TEST] Admin orders popup regression completed successfully.');
  } catch (error) {
