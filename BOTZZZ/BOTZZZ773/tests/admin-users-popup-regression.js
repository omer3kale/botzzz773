...
async function seedAdminSession(context) {
  ...
  await context.addInitScript(([sessionToken, sessionUser]) => {
    try {
      ...
      console.warn('[ADMIN USERS POPUP TEST] Failed to seed session:', error);
    }
  }, [token, user]);
}

async function openAdminUsersPopup(page, context) {
  const launcherId = await page.evaluate(() => {
    const button = document.createElement('button');
    const id = `admin-users-popup-launcher-${Date.now()}`;
    button.id = id;
    button.type = 'button';
    button.style.position = 'absolute';
    button.style.left = '-9999px';
    button.textContent = 'Open Admin Users Popup';
    button.addEventListener('click', () => {
      window.open('/admin/users.html?popup=1', 'botzzz-admin-users', 'width=1280,height=820,resizable=yes,scrollbars=yes');
    });
...
async function run() {
  ...
  try {
    ...
    console.log('[TEST] Starting admin users popup verification...');
    await page.goto(new URL('/admin/users.html', BASE_URL).toString(), { waitUntil: 'domcontentloaded' });

    const popup = await openAdminUsersPopup(page, context);
    const popupUrl = popup.url();

    if (!popupUrl.includes('/admin/users.html') || !popupUrl.includes('popup=1')) {
      throw new Error('Admin users popup URL missing popup parameter.');
    }

    const popupMode = await popup.evaluate(() => document.body.classList.contains('popup-mode'));
    if (!popupMode) {
      throw new Error('Admin users popup did not enable popup-mode.');
    }
    ...
    console.log('[TEST] Admin users popup regression completed successfully.');
  } catch (error) {
